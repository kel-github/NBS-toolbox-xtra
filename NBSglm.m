...
if isempty(ind_nuisance)
    ...
    b=zeros(length(ind_nuisance),M); % b = nuisance predictors x M (number of GLMs)
    resid_y=zeros(n,M); 
    b=GLM.X(:,ind_nuisance)\GLM.y; % https://www.mathworks.com/matlabcentral/answers/91458-when-should-i-use-x-a-b-and-when-should-i-use-x-inv-a-b
...
for i=1:GLM.perms+1
    ...
    if ~isempty(ind_nuisance)
        y_perm=resid_y+[GLM.X(:,ind_nuisance)]*b; % adding the variance explained by the nuisance regressors back into the data
    end
       
    b_perm=zeros(p,M);
    b_perm=GLM.X\y_perm; % solve the system with all regressors
